...
def data_over_time(df, col):
    # Drop duplicates for unique (Year, col) combinations
    temp_df = df.drop_duplicates(subset=['Year', col])

    # Group by Year and count unique entries of `col`
    year_counts = temp_df.groupby('Year')[col].nunique().reset_index()

    # Rename for uniformity
    year_counts.rename(columns={col: 'Count'}, inplace=True)
...
def most_successful_countrywise(df, country):
    ...
    # Drop duplicates to keep one row per athlete
    merged_df = merged_df.drop_duplicates(subset='Name')
